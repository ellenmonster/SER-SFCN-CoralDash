...
observeEvent(eventExpr = input$button_update_cover, {
  ...
  shiny::req(!is.null(rv$PC_Site), !is.null(input$sel_lev), !is.null(input$sel_geogscale), !is.null(input$sel_sublev), !is.null(input$sel_showRSS), !is.null(input$sel_episodic), !is.null(input$sel_error), !is.null(input$sel_splitplot))
 
  dat_PC <- subset(rv$PC_Site[[input$sel_lev]], SiteLev == input$sel_geogscale & Sublevel %in% input$sel_sublev & RSS %in% input$sel_showRSS)
  ...
  if(input$sel_metric == "Median" & input$sel_error == "95%CI")  # not the most efficient way, but it's fail-safe
    dat_PC2 <- dat_PC %>%
    select(RSS, Sublevel, Year, SurvDate, NumTransect, y_val = median, y_low = med_low95, y_high = med_high95)
  ...
  if(input$sel_metric == "Median" & input$sel_error == "SE")
    ...
    select(RSS, Sublevel, Year, SurvDate, NumTransect, y_val = median, y_low, y_high)
...
saveRDS(isolate(reactiveValuesToList(rv_param_temp)), "temp_rv_param_temp.RDS"),
    ...
    radioButtons("sel_geogscale", 
             label = "Geographic scale to summarize", 
    ...
    tags$div(renderText({ 
      shiny::req(!is.null(input$sel_geogscale))
      
      if(input$sel_geogscale == "RepSiteSurvID") {
    ...
    renderUI({
      shiny::req(!is.null(rv$mapdat), !is.null(input$sel_geogscale), !is.null(input$PCactive))
      
      RSS_choices <- rv$mapdat %>%
        dplyr::filter(if(input$PCactive == TRUE) IsActive == 1 else TRUE) %>%
        {if(input$sel_geogscale == "SiteSurvID") dplyr::select(., Site) else dplyr::select(., ReportingSite)} %>%
        dplyr::distinct() %>%
        pull() %>%
        as.character()

      checkboxGroupInput("sel_showRSS", label = paste0("Show results for these ", ifelse(input$sel_geogscale == "RepSiteSurvID", "reporting sites", "sites")), choices = RSS_choices, selected = if(!is.null(input$sel_showRSS) & all(input$sel_showRSS %in% RSS_choices)) {input$sel_showRSS} else {RSS_choices}, inline = FALSE) # if selections already made and they are a subset of RSS_choices, then use those selections; otherwise, use RSS_choices as default
      }),
    ...
    # Select sublevels
    # avail_sublevs <- reactive({
    #   if(!is.null(rv$PC_Site) & !is.null(input$sel_lev) & !is.null(input$sel_geogscale) & !is.null(input$sel_showRSS) & !is.null(input$sel_episodic)) {
    # 
    #   avail_temp <- subset(rv$PC_Site[[input$sel_lev]], SiteLev == input$sel_geogscale & RSS %in% input$sel_showRSS) %>%
    ...
    renderUI({
  shiny::req(!is.null(rv$PC_Site), !is.null(input$sel_lev), !is.null(input$sel_geogscale), !is.null(input$sel_showRSS), !is.null(input$sel_episodic))
      rv_param_temp$PC_avail_sublevs <- subset(rv$PC_Site[[input$sel_lev]], SiteLev == input$sel_geogscale & RSS %in% input$sel_showRSS) %>%
        dplyr::filter(if(input$sel_episodic == FALSE) Purpose == "Annual" else TRUE) %>%
        pull(Sublevel) %>%
        as.character() %>%
        unique()
...
renderUI({
  ...
  selectInput("sel_splitplot", label = "In a single plot, show: ", choices = c("One location, one sublevel" = "one_loc_one_sub", "One sublevel, many locations" = "one_sub_many_loc", "One location, many sublevels" = "one_loc_many_sub"), selected = switch(is.null(input$sel_splitplot)+1, input$sel_splitplot, "one_loc_one_sub"), width = "95%")
})
