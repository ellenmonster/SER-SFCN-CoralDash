...
observeEvent(eventExpr = input$button_update_disease, {
  ...
  # Filter colony counts data for the user-specified site and calculate avg count
   ...
   # Disease data with associated colony numbers. Can only calculate prevalence for the years where disease colony numbers recorded for all disease entries
   ...
   disease_prev_transect <- disease_cols_complete %>%
    ...
    disease_prev_transect$TransectDiseased[is.na(disease_prev_transect$TransectDiseased)]<- 0 # if there is a non-NA StonyCoralCountAVG, convert any NA disease count to zero
...
renderUI({
  shiny::req(!is.null(rv_param_temp$disease_prev_subdat), input$disease_free_ymax == FALSE) # this option is only shown when y-axes scales are not allowed to vary among plots
  top_count <-max(rv_param_temp$disease_prev_subdat$SiteDiseased, na.rm = TRUE) %>% plyr::round_any(., 5, f = ceiling)
...
renderUI({
  shiny::req(!is.null(rv_param_temp$disease_prev_subdat_transect), input$disease_free_ymax == FALSE) # this option is only shown when y-axes scales are not allowed to vary among plots
  top_prev <-max(rv_param_temp$disease_prev_subdat_transect$TransectPrev, na.rm = TRUE) %>% plyr::round_any(., 0.2, f = ceiling)
...
output$out_disease_count_plotly <- plotly::renderPlotly({
  ...
  shiny::validate(
    need(!is.null(rv_param_temp$disease_prev_subdat) & !is.null(rv_param_temp$disease_prev_subdat_transect), message = "From 'Data Filter Options' on the left, please select site(s) and disease(s), then press the green 'UPDATE DISEASE SUMMARIES' button"))
