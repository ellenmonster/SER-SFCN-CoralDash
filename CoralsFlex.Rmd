...
6. When data processing is completed, a summary data file will be output to the current working directory as an RDS with the name '(your prefix)_coralsummary.RDS'.

#### Instructions for uploading a summary RDS file

7. Browse to and upload the '(your prefix)_coralsummary.RDS' file, to generate summaries in the plot and table tabs.
...
fileInput("sitesfilenam", label = "Select a sites CSV file", accept = c('text/csv', 'text/comma-separated-values', '.csv'))

actionButton("button_import", "IMPORT AND PROCESS NEW DATA", width = "300px")
       ...
       if(input$sel_MapMetric == "BleachRC") {
        chartmap <- basemap %>%
          addMinicharts(
            ...
            colorPalette = c("yellow", "gray25", "gray45", "gray55", "gray75", "gray92"),
            opacity = .85
          )
      }
     }
