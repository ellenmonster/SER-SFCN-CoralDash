...
rv <- reactiveValues(raw_dat = NULL, map_dat = NULL, groups_df = NULL, warn_list = NULL, transect_counts_df = NULL, cover_CIs_df = NULL, bleach_CIs_df = NULL)

rv_param_temp <- reactiveValues(dat_PC = NULL, avail_sublevs = NULL, sel_showRSS = NULL, sel_lev = NULL, sel_sublev = NULL, transects_subdat = data.frame(), bleach_subdat = data.frame())
...
```{r functions}
# FuncPlotlyHeatMap <- function(dat, y_name, facet_by_name, legend_name, is_discrete = FALSE) {
#   # Function to create plotly heatmaps
#   #
#   # Args:
#   #   dat: Data frame with these columns: SurvDate (for x-axis), Y, FillCol, HoverLabel. 'X' should be factor class
...
#   template <- dat %>%
#     expand(SurvDate, Y) %>%
#     left_join(dat[c("Y", "SurvDate", "FillCol", "HoverLabel")], by = c("Y", "SurvDate")) %>%
#     dplyr::mutate(Y = factor(Y, levels = sort(unique(Y)))) %>%
#     dplyr::arrange(Y, SurvDate)
#     
#   
#   # Calculate total and relative heights
#   total_page_ht <- length(levels(dat$Y))+55
#  
#   # Create color scale
#   min_colorval <- min(template$FillCol, na.rm = TRUE)
#   max_colorval <- max(template$FillCol, na.rm = TRUE) 
...
#     get_legend(
#       ggplot(template, aes(x = SurvDate, y = Y, fill = FillCol)) +
#         geom_tile() +
...
#   )
#   
#   mat_values <- matrix(pull(template, FillCol),
#                        nrow=length(levels(template$Y)),
#                        byrow = TRUE,
#                        dimnames = list(
#                          levels(template$Y),
#                          sort(unique(SurvDate))
#                          )
#                        )
#   mat_hover <- matrix(pull(template, HoverLabel),
#                       nrow=length(levels(template$Y)),
#                        byrow = TRUE)
#   shiny::req(!is.null(template), !is.null(mat_values), !is.null(mat_hover))
#   
#   # Create heatmap <<<<<<<<<<<<<<<<<< PICK UP FROM HERE
#     # none of the margin arguments are helpful
#     if(sum(!is.na(template$FillCol)) > 0) {
...
#           r=40)
#           )
#   return_list <- list(legend = shared_legend, plot = plotly_heatmap)
#   return(return_list)
# }

FuncPlotPointCI <- function(dat, denom, sel_error, sel_splitplot, PCy_max, y_text, PC_blackwhite) {
  ...
  if(sel_splitplot == "one_loc_one_sub") { # one plot per loc-sub
    ...
    p <- 
      ggplot(data = plot_dat, aes_string(x = "SurvDate", y = "EstimCov", colour = ifelse(sel_splitplot == "one_loc_many_sub", "NumerLevel", "RSS"))) 
  ...
  p <- p +
    ...
    facet_wrap(as.formula(facet_vars), scales = "free_y", ncol = 1) +
...
wellPanel(
  ...
  renderUI({
    shiny::req(!is.null(rv$map_dat), !is.null(input$sel_raw_sitescale), !is.null(input$raw_RSS_active))
    
    raw_RSS_choices <- rv$map_dat %>%
      dplyr::filter(if(input$raw_RSS_active == TRUE) IsActive == 1 else TRUE) %>%
      {if(input$sel_raw_sitescale == "Site") dplyr::select(., Site) else dplyr::select(., ReportingSite)} %>%
      dplyr::distinct() %>%
      pull() %>%
      as.character()
    ...
    selectInput("sel_raw_showRSS", 
                       label = paste0("Show results for this ", ifelse(input$sel_raw_sitescale == "ReportingSite", "reporting site", "site")),
...
output$out_cover_plot <- plotly::renderPlotly({
  shiny::validate(
    need(!is.null(rv_param_temp$dat_PC), message = "From 'Data Filter Options' on the left, please select a grouping level and sublevel(s), then press the green 'UPDATE COVER SUMMARIES' button"))
...
FuncPlotPointCI(dat = rv_param_temp$dat_PC, denom = "TransectCount", sel_error = input$sel_error, sel_splitplot = input$sel_splitplot, PCy_max = input$PCy_max, y_text = paste0("Mean % Cover Per Transect (", input$sel_error, ")"), PC_blackwhite = input$PC_blackwhite)
...
output$out_RC_plot <- plotly::renderPlotly({
  ...
  shiny::validate(
    need(!is.null(rv_param_temp$dat_PC), message = "Relative cover is estimated when the grouping level is 'Functional Group' or 'Taxon' with at least one selected sublevel. Press the green 'UPDATE COVER SUMMARIES' button after selection."))
  ...
  FuncPlotPointCI(dat = rv_param_temp$dat_PC, denom = "CategoryCount", sel_error = input$sel_error, sel_splitplot = input$sel_splitplot, PCy_max = input$PCy_max, y_text = paste0("Mean Relative Cover in Group (", input$sel_error, ")"), PC_blackwhite = input$PC_blackwhite)
...
wellPanel(
  ...
  renderUI({
    shiny::req(!is.null(rv$map_dat), !is.null(input$sel_bleach_sitescale), !is.null(input$bleach_RSS_active))
    
    bleach_RSS_choices <- rv$map_dat %>%
      dplyr::filter(if(input$bleach_RSS_active == TRUE) IsActive == 1 else TRUE) %>%
      {if(input$sel_bleach_sitescale == "Site") dplyr::select(., Site) else dplyr::select(., ReportingSite)} %>%
      dplyr::distinct() %>%
      pull() %>%
      as.character()
    ...
    selectInput("sel_bleach_showRSS", 
                label = paste0("Show results for this ", ifelse(input$sel_bleach_sitescale == "ReportingSite", "reporting site", "site")),
...
output$out_bleach_PC_plot <- plotly::renderPlotly({
  ...
  plot_bleachPC <- FuncPlotBleach(dat = subset(rv_param_temp$bleach_subdat, DenomGroup == "TransectCount"))
...
output$out_bleach_RC_plot <- plotly::renderPlotly({
  ...
  plot_bleachRC <- FuncPlotBleach(dat = subset(rv_param_temp$bleach_subdat, DenomGroup == "CategoryCount"))
