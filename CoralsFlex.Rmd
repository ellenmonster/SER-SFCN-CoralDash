...
packages <- c("flexdashboard", "shiny", "knitr", "gridExtra", "shinyWidgets", "leaflet", "plotly", "DT", "parallel", "rtf", "lubridate", "leaflet.minicharts", "multidplyr", "tidyverse", "htmlwidgets", "RSQLite", "multidplyr", "Rcpp", "reactable")
...
dat_PC3 <- reactive({
  ...
  # saveRDS(dat_PC2, "temp_dat_PC3.RDS") # <<<<<<<<<<<<<<<<<<<
  dat_PC2
    }
})
...
renderTable({
  validate(need(!is.null(dat_RC3()), message = FALSE))
    RCsamp <- dat_RC3() %>%
      ...
      distinct() %>%
      arrange(SurvDate)
    RCsamp$SurvDate <- lubridate::ymd(RCsamp$SurvDate)
    RCsamp
...
renderTable({
  ...
  dat_RCtab$SurvDate <- lubridate::ymd(dat_RCtab$SurvDate)
  dat_RCtab
  }, align = "c", na = "--", include.rownames = FALSE
  )
```
