...
pkgList <- c("flexdashboard", "shiny", "knitr", "gridExtra", "cowplot", "shinyWidgets", "leaflet", "plotly", "DT", "rtf", "lubridate", "leaflet.minicharts", "tidyverse", "magrittr", "htmlwidgets", "RSQLite", "multidplyr", "Rcpp", "scales", "patchwork", "zoo")
...
rv_param_temp <- reactiveValues(cover_subdat = NULL, avail_sublevs = NULL, sel_showRSS = NULL, sel_bleach_showRSS = NULL, sel_lev = NULL, sel_sublev = NULL, avail_diseases = NULL, transects_subdat = NULL, sel_raw_dates = NULL, bleach_subdat = NULL, disease_prev_subdat = NULL, disease_prev_subdat_transect = NULL, sel_disease_showRSS = NULL, cover_plot = NULL, bleach_plot = NULL)
...
FuncPlotPointCI <- function(dat, denom, sel_error, sel_splitplot, free_ymax, set_ymax, y_text, which_showRSS, PC_blackwhite) {
  ...
  # assign colors
    ...
    if(PC_blackwhite) {
      p <- p + scale_colour_grey(guide = guide_legend(override.aes = list(size=2)))
    } else {
      if(which_showRSS == "bleach") {
        p <- p + 
          scale_color_manual(values = c("UNBL" = "gray25", "BL1" = "gray45", "BL2" = "gray55", "BL3" = "gray75", "BL4" = "gray92"), guide = guide_legend(override.aes = list(size=1.25)))
      } else { # for cover plots
        p <- p +
        scale_color_manual(values = cbp1_assigned, guide = guide_legend(override.aes = list(size=1.25)))
      }
    }
  ...
  cov_legend <- ggpubr::as_ggplot(cowplot::get_legend(p))
      
  facet_ht <- 250 # 215
  add_on_ht <- 150 # 150, this is for x-axis
  num_locs <- case_when(
    which_showRSS == "cover" ~ length(rv_param_temp$sel_showRSS),
    which_showRSS == "bleach" ~ length(rv_param_temp$sel_bleach_showRSS))
  num_sub <- case_when(
    which_showRSS == "cover" ~ length(rv_param_temp$sel_sublev),
    which_showRSS == "bleach" ~ length(levels(dat$NumerLevel)))
  ...
  return_list <- list(plot = gp, legend = cov_legend)
  
  return(return_list)
}

FuncPlotPointDisease <- function(dat, metric, free_ymax, y_text, disease_by_transect) {
  ...
  if(disease_by_transect == TRUE){ # if separate line for each transect...
    p <- 
      ggplot(data = plot_dat, aes(x = SurvDate, y = PlotMetric, colour = Transect)) +
      scale_color_viridis_d(name = metric, direction=-1, guide = guide_legend(override.aes = list(size=1.25))) +
      geom_point(aes(text = paste0("Reporting Site: ", ReportingSite, "<br>Site: ", Site, "<br>Survey Date: ", SurvDate, "<br>Transect: ", Transect, "<br>Y-VALUE: ", PlotMetric, "<br><br>Number of Diseased Colonies: ", TransectDiseased, "<br>Number of Colonies: ", StonyCoralCount1, "<br>Disease Prevalence: ", TransectPrev))) + # aesthetic text is for ggplotly tooltip. Gives warning that it's not recognized but it still works
    geom_line(aes(text = paste0("Reporting Site: ", ReportingSite, "<br>Site: ", Site, "<br>Survey Date: ", SurvDate, "<br>Transect: ", Transect, "<br>Y-VALUE: ", PlotMetric, "<br><br>Number of Diseased Colonies: ", TransectDiseased, "<br>Number of Colonies: ", StonyCoralCount1, "<br>Disease Prevalence: ", TransectPrev)), group = "Transect") # need to add this grouping to geom_line, otherwise the text will cause it to group incorrectly. Need to add text b/c otherwise the line will not show when plotly tooltip is set to "text"
  } else {
    p <- 
      ggplot(data = plot_dat, aes(x = SurvDate, y = PlotMetric)) +
      geom_point(aes(text = paste0("Reporting Site: ", ReportingSite, "<br>Site: ", Site, "<br>Survey Date: ", SurvDate, "<br>Y-VALUE: ", PlotMetric, "<br><br>Number of Diseased Colonies: ", SiteDiseased, "<br>Number of Colonies: ", SiteStonyCoralCount1, "<br>Disease Prevalence: ", SitePrev))) + # aesthetic text is for ggplotly tooltip. Gives warning that it's not recognized but it still works
      geom_line(aes(text = paste0("Reporting Site: ", ReportingSite, "<br>Site: ", Site, "<br>Survey Date: ", SurvDate, "<br>Y-VALUE: ", PlotMetric, "<br><br>Number of Diseased Colonies: ", SiteDiseased, "<br>Number of Colonies: ", SiteStonyCoralCount1, "<br>Disease Prevalence: ", SitePrev)), group = "RSS")
...
FuncPlotBleach <- function(dat, yaxis_title) {
  ...
  plots_list <- lapply(sort(unique(dat$RSS)), function(i) {
    ...
    ggplot(subset(plot_dat, RSS == i), aes(x = SurvDate, y = EstimCov, fill = NumerLevel)) +
    geom_col(colour="black", size = 0.1, position = position_stack(reverse = TRUE)) +
    scale_fill_manual(values = c("UNBL" = "gray25", "BL1" = "gray45", "BL2" = "gray55", "BL3" = "gray75", "BL4" = "gray92"), guide = "none") +
    scale_y_continuous(expand = c(0, 0)) +
    labs(title = " ", subtitle = paste0("Coral Bleaching at Site:  ", i), y = yaxis_title) + # adding blank title as hack to increase spacing between plots
  ...
  plots_page <- patchwork::wrap_plots(plots_list, ncol = 1) +
    patchwork::plot_annotation(
      title = "From Oct. 2005 to present, coral data with NA for 'Bleaching Code' are considered 'Unbleached' (UNBL)",
      theme = theme(
...
FuncPlotDisease <- function(dat, y_col, yaxis_title, title_text) {
  # NOT interactive because with interactive subplots of facet plots, the y-axis was not showing in the correct location 
  # Function to stacked bar plots of site totals
  #
  # Args:
  ...
  plots_list <- lapply(sort(unique(dat$RSS)), function(i) {
    ...
    ggplot(subset(plot_dat, RSS == i), aes_string(x = "SurvDate", y = y_col, fill = "DiseaseStatus")) +
      geom_col(colour="black", size = 0.1, position = position_stack(reverse = TRUE)) +
      scale_fill_manual(values = c("Diseased" = "red", "NotDiseased" = "black", "Unknown" = "yellow"), guide = "none") +
    scale_y_continuous(expand = c(0, 0)) +
...
observeEvent(eventExpr = input$button_update_bleach, {
  ...
  shiny::req(!is.null(rv$bleach_CIs_df), !is.null(input$sel_bleach_sitescale), !is.null(input$sel_bleach_showRSS), !is.null(input$sel_bleach_episodic), !is.null(input$sel_bleach_error))
  
  rv_param_temp$sel_bleach_showRSS <- rv_param_temp$bleach_plot <- NULL
  ...
  rv_param_temp$bleach_subdat <- rv$bleach_CIs_df %>%
    ...
    dplyr::mutate(
      ...
      NumerLevel = factor(NumerLevel, levels = c("NONCORAL", "UNBL", "BL1", "BL2", "BL3", "BL4"))) %>%
  ...
  # Filter colony counts data for the user-specified site
   master_counts <- rv$colcounts_df %>%
     ...
     dplyr::mutate(Site_Transect = paste(Site, Transect, sep = "_"),
                   StonyCoralCountDIFF = StonyCoralCount2 - StonyCoralCount1,
                   FireCoralCountDIFF = FireCoralCount2 - FireCoralCount1)
   ...
   # Transect-level disease counts and prevalence, per survey event (in case it's useful to see the data by transect)
   ...
   disease_prev_transect %<>%
     dplyr::mutate(
       ...
       TransectPrev = round(100* (TransectDiseased / StonyCoralCount1), 1)
       ) %>%
     ungroup()
   
   # Site-level disease counts and prevalence (summed across transects for a survey event)
   disease_prev_subdat <- disease_prev_transect %>%
     dplyr::select(-TransectSurveyID, -Transect) %>%
     dplyr::group_by(across(c(-Site_Transect, -TransectDiseased, -TransectNotDiseased, -StonyCoralCount1, -StonyCoralCount2, -StonyCoralCountDIFF, -FireCoralCount1, -FireCoralCount2, -FireCoralCountDIFF, -TransectPrev))) %>%
     dplyr::summarize(
       SiteDiseased = sum(TransectDiseased),
       SiteNotDiseased = sum(TransectNotDiseased),
       SiteStonyCoralCount1 = sum(StonyCoralCount1),
       SiteStonyCoralCount2 = sum(StonyCoralCount2),
       SiteStonyCoralCountDIFF = SiteStonyCoralCount2 - SiteStonyCoralCount1,
       SiteFireCoralCount1 = sum(FireCoralCount1),
       SiteFireCoralCount2 = sum(FireCoralCount2),
       SiteFireCoralCountDIFF = SiteFireCoralCount2 - SiteFireCoralCount1,
       SitePrev = round(100* (SiteDiseased / SiteStonyCoralCount1), 1), 
       ) %>% 
     ungroup()
  
   rv_param_temp$sel_disease_showRSS <- input$sel_disease_showRSS
   rv_param_temp$disease_prev_subdat <- disease_prev_subdat
   rv_param_temp$disease_prev_subdat_transect <- disease_prev_transect
   })
```
...
output$out_transects_plotly <- plotly::renderPlotly({
  ...
  p <- ggplot(rv_param_temp$transects_subdat, aes(x = SurvDate, y = PercCov)) +
    labs(title = paste0("% Cover per Transect for ", toupper(unique(rv_param_temp$transects_subdat$Category)), " at ", unique(rv_param_temp$transects_subdat$RSS))) +
    geom_point() +
    geom_line() +
...
checkboxInput("free_ymax", label = "Allow y-axes scales to vary among plots", value = FALSE)

renderUI({
  shiny::req(input$free_ymax == FALSE) # this option is only shown when y-axes scales are not allowed to vary among plots
  numericInput("set_PC_ymax", label = "Set y-axis maximum for % cover", min = 1, max = 105, step = 1, value = switch(is.null(input$set_PC_ymax)+1, input$set_PC_ymax, NULL))
})

renderUI({
  shiny::req(input$free_ymax == FALSE & input$sel_lev != "Category" & !is.null(rv$cover_CIs_df)) # this option is only shown when y-axes scales are not allowed to vary among plots
  top_val <-max(rv$cover_CIs_df %>% dplyr::filter(DenomGroup == "CategoryCount") %>% dplyr::select(`90%CI_high`), na.rm = TRUE)
  numericInput("set_RC_ymax", label = "Set y-axis maximum for relative cover", min = 1, max = plyr::round_any(max(rv$cover_CIs_df$`90%CI_high`[rv$cover_CIs_df$DenomGroup == "CategoryCount"], na.rm = TRUE), 5, f = ceiling), step = 1, value = switch(is.null(input$set_RC_ymax)+1, input$set_RC_ymax, NULL))
})
...
* To set y-axis maxima in graphs, uncheck 'Allow y-axes scales to vary among plots' in left-side panel, then enter maximum y-values. To set x-axis range in graphs, click and drag mouse over the desired range on x-axis. Double-click the graph to return to full x-axis range.

####
```{r cover_legend}

renderPlot({ # separate validation statements so they are checked in sequence
  shiny::req(!is.null(rv_param_temp$cover_subdat), !is.null(rv_param_temp$cover_plot$legend))
  if(any(!rv_param_temp$sel_showRSS %in% rv_param_temp$cover_subdat$RSS, !rv_param_temp$sel_sublev %in% rv_param_temp$cover_subdat$NumerLevel)) { # this if-statement forces the plot to be reset to blank when these conditions apply
    
    cover_legend <- ggplot(dat = data.frame()) + theme_minimal()
  } else {
    
    cover_legend <-
      rv_param_temp$cover_plot$legend # shared legend for dashboard
  }
  grid.arrange(cover_legend)
}, height = 40)
```
...
rv_param_temp$cover_plot <- FuncPlotPointCI(dat = rv_param_temp$cover_subdat, denom = "TransectCount", sel_error = input$sel_error, sel_splitplot = input$sel_splitplot, free_ymax = input$free_ymax, set_ymax = input$set_PC_ymax, y_text = paste0("Mean % Cover (", input$sel_error, ")"), which_showRSS = "cover", PC_blackwhite = input$PC_blackwhite)

rv_param_temp$cover_plot$plot %>% hide_legend(.)
})
...
output$out_RC_plotly <- plotly::renderPlotly({
  ...
  shiny::req(!is.null(input$sel_splitplot), !is.null(input$PC_blackwhite), !is.null(input$sel_error), !is.null(rv_param_temp$sel_showRSS), !is.null(rv_param_temp$sel_lev), !is.null(rv_param_temp$sel_sublev), !is.null(input$free_ymax))
  ...
  FuncPlotPointCI(dat = rv_param_temp$cover_subdat, denom = "CategoryCount", sel_error = input$sel_error, sel_splitplot = input$sel_splitplot, free_ymax = input$free_ymax, set_ymax = input$set_RC_ymax, y_text = paste0("Mean % Relative Cover (", input$sel_error, ")"), which_showRSS = "cover", PC_blackwhite = input$PC_blackwhite)$plot %>% hide_legend(.)
...
* Export buttons only export records visible on the current page. To export ALL data (or all filtered data), select 'Show [All] entries' to make all records visible on current page and then click the export button (e.g., Excel). For large data files, export may take a few seconds. 
...
Coral Bleaching (line graphs w/error bars) {data-navmenu="Coral Bleaching per Site"}
======================================
Inputs {.sidebar data-width=300}
-------------------------------------
...
renderUI({
  radioButtons("sel_bleach_error",
               label = "Bootstrapped error bars to plot",
               choiceNames = list("90%CI", "68%CI (~1SE)", "50%CI"),
               choiceValues = list("90%CI", "68%CI", "50%CI"),
               selected = switch(is.null(input$sel_bleach_error)+1, input$sel_bleach_error, "68%CI"))
})

checkboxInput("bleach_free_ymax", label = "Allow y-axes scales to vary among plots", value = FALSE)

renderUI({
  shiny::req(input$bleach_free_ymax == FALSE) # this option is only shown when y-axes scales are not allowed to vary among plots
  numericInput("set_bleach_PC_ymax", label = "Set y-axis maximum for % cover", min = 1, max = 105, step = 1, value = switch(is.null(input$set_bleach_PC_ymax)+1, input$set_bleach_PC_ymax, NULL))
})

renderUI({
  shiny::req(input$bleach_free_ymax == FALSE & !is.null(rv$bleach_CIs_df)) # this option is only shown when y-axes scales are not allowed to vary among plots
  top_val <-max(rv$bleach_CIs_df %>% dplyr::filter(DenomGroup == "CategoryCount") %>% dplyr::select(`90%CI_high`), na.rm = TRUE)
  numericInput("set_bleach_RC_ymax", label = "Set y-axis maximum for relative cover", min = 1, max = plyr::round_any(max(rv$bleach_CIs_df$`90%CI_high`[rv$bleach_CIs_df$DenomGroup == "CategoryCount"], na.rm = TRUE), 5, f = ceiling), step = 1, value = switch(is.null(input$set_bleach_RC_ymax)+1, input$set_bleach_RC_ymax, NULL))
})
```

Coral Bleaching
-------------------------------------
#### <font size="3">**These line graphs show % cover by bleach level (left-side panel) and relative % cover by bleach level (right-side panel) for each selected site**</font>

* Relative % cover is calculated as % of coral cover in each bleach level
...
renderPlot({ # separate validation statements so they are checked in sequence
  shiny::req(!is.null(rv_param_temp$bleach_subdat), !is.null(rv_param_temp$bleach_plot$legend))
  if(any(!rv_param_temp$sel_bleach_showRSS %in% rv_param_temp$bleach_subdat$RSS)) { # this if-statement forces the plot to be reset to blank when these conditions apply
    
    bleach_legend <- ggplot(dat = data.frame()) + theme_minimal()
  } else {
    bleach_legend <-
      rv_param_temp$bleach_plot$legend # shared legend for dashboard
  }
  grid.arrange(bleach_legend)
}, height = 40)
```
...
<br>
```{r plot_bleachPC}
output$out_bleachPC_plotly <- plotly::renderPlotly({
  
  shiny::validate(
    need(!is.null(rv$raw_dat), message = "Please import new data or load an existing summary RDS file"))
  
  shiny::validate(
    need(!is.null(rv_param_temp$bleach_subdat), message = "From 'Data Filter Options' on the left, please select site(s), then press the green 'UPDATE BLEACH SUMMARIES' button"))
 
  shiny::req(!is.null(input$sel_bleach_error), !is.null(input$bleach_free_ymax), !is.null(rv_param_temp$sel_bleach_showRSS))
  
  rv_param_temp$bleach_plot <- FuncPlotPointCI(dat = rv_param_temp$bleach_subdat, denom = "TransectCount", sel_error = input$sel_bleach_error, sel_splitplot = "one_loc_many_sub", free_ymax = input$bleach_free_ymax, set_ymax = input$set_bleach_PC_ymax, y_text = paste0("% Cover of Site (", input$sel_bleach_error, ")"), which_showRSS = "bleach", PC_blackwhite = FALSE)
  
  rv_param_temp$bleach_plot$plot %>% hide_legend(.)
})

plotlyOutput("out_bleachPC_plotly")
```
...
<br>
```{r plot_bleachRC}
output$out_bleachRC_plotly <- plotly::renderPlotly({
  ...
  FuncPlotPointCI(dat = rv_param_temp$bleach_subdat, denom = "CategoryCount", sel_error = input$sel_bleach_error, sel_splitplot = "one_loc_many_sub", free_ymax = input$bleach_free_ymax, set_ymax = input$set_bleach_RC_ymax, y_text = paste0("% Cover of Corals (", input$sel_bleach_error, ")"), which_showRSS = "bleach", PC_blackwhite = FALSE)$plot %>% hide_legend(.)
  
})

plotlyOutput("out_bleachRC_plotly")
```
</div>
</div>

% Coral Bleaching (stacked bar plots) {data-navmenu="Coral Bleaching per Site"}
======================================
% Cover and Relative Cover per Site
-------------------------------------
#### <font size="3">**These stacked bar plots show % cover by bleach level (left-side panel) and relative % cover by bleach level (right-side panel) for each selected site and category/functional group/taxon level [PLOTS ARE SLOW TO DRAW - PLEASE BE PATIENT]**</font>
...
<br>
```{r barplot_bleachPC}
output$out_bleach_stackplot <- renderPlot({
  
  shiny::req(!is.null(rv$bleach_CIs_df), !is.null(input$bleach_RSS_active), !is.null(input$sel_bleach_episodic), !is.null(input$sel_bleach_sitescale))
...
# saveRDS(isolate(reactiveValuesToList(rv_param_temp)), "temp_rv_param_temp.RDS")
  ...
  FuncPlotBleach(dat = subset(rv_param_temp$bleach_subdat, DenomGroup == "TransectCount"), yaxis_title = "% Cover of Site")
},
      height = function(){shiny::validate(need(!is.null(rv$raw_dat), message = "Please import new data or load an existing summary RDS file"));shiny::validate(need(!is.null(rv_param_temp$bleach_subdat), message = "From 'Data Filter Options' on the line graphs page, please select site(s), then press the green 'UPDATE BLEACH SUMMARIES' button"));shiny::req(!is.null(rv_param_temp$sel_bleach_showRSS));length(rv_param_temp$sel_bleach_showRSS)*350}
  )

plotOutput("out_bleach_stackplot")
```
...
<br>
```{r barplot_bleachRC}
output$out_bleach_stackplot2 <- renderPlot({
    
    shiny::req(!is.null(rv$bleach_CIs_df), !is.null(input$bleach_RSS_active), !is.null(input$sel_bleach_episodic), !is.null(input$sel_bleach_sitescale))
  
  FuncPlotBleach(dat = subset(rv_param_temp$bleach_subdat, DenomGroup == "CategoryCount"), yaxis_title = "% Cover of Corals")
},
      height = function(){shiny::validate(need(!is.null(rv$raw_dat), message = "Please import new data or load an existing summary RDS file"));shiny::validate(need(!is.null(rv_param_temp$bleach_subdat), message = "From 'Data Filter Options' on the line graphs page, please select site(s), then press the green 'UPDATE BLEACH SUMMARIES' button"));shiny::req(!is.null(rv_param_temp$sel_bleach_showRSS));length(rv_param_temp$sel_bleach_showRSS)*350}
  )

# tags$style("#out_bleach_stackplot2{height:100vh;overflow-y:scroll}")
plotOutput("out_bleach_stackplot2")

```
</div>
</div>
  
% Bleaching (table of %cover w/CI) {data-navmenu="Coral Bleaching per Site"}
======================================
#### <font size="3">**This table corresponds with the left-side panel of the coral bleaching line graphs. For each site, the data shown are % cover by bleach level and various bootstrapped confidence intervals.**</font>
...
Relative % Bleaching (table of relative cover w/CI) {data-navmenu="Coral Bleaching per Site"}
======================================
#### <font size="3">**This table corresponds with the right-side panel of the coral bleaching line graphs. For each site, the data shown are relative % cover by bleach level and various bootstrapped confidence intervals.**</font>
...
Coral Disease Counts & Prevalence (line graphs) {data-navmenu="Coral Disease per Site"}
======================================
Inputs {.sidebar data-width=300}
-------------------------------------
...
wellPanel(
  ...
  renderUI({
    ...
    disease_RSS_choices <- rv$disease_df %>%
      ...
      {if(input$sel_disease_sitescale == "Site") dplyr::pull(., Site) else dplyr::pull(., ReportingSite)} %>% unique(.)
...
checkboxInput("disease_free_ymax", label = "Allow y-axes scales to vary among plots", value = FALSE)
```

Coral Disease
-------------------------------------
#### <font size="3">**These line graphs show # of diseased colonies (left-side panel) and disease prevalence (right-side panel) by transect or site**</font>
  
* Disease prevalence is calculated as 100 * (# of colonies with disease / # of colonies)

* Counts and prevalence are summaries of the survey data, without estimates of statistical error.

####
```{r disease_legend}
# This is only relevant when disease_by_transect is TRUE
renderPlot({ # separate validation statements so they are checked in sequence
  shiny::req(!is.null(rv_param_temp$disease_prev_subdat_transect), !is.null(input$disease_by_transect), !is.null(rv_param_temp$disease_prev_plot$legend))
  
  if(input$disease_by_transect == FALSE | any(!rv_param_temp$sel_disease_showRSS %in% rv_param_temp$disease_prev_subdat$RSS)) { # this if-statement forces the plot to be reset to blank when these conditions apply
    
    disease_legend <- ggplot(dat = data.frame()) + theme_minimal()
  } else {
    disease_legend <-
      rv_param_temp$disease_prev_plot$legend # shared legend for dashboard
  }
  grid.arrange(disease_legend)
}, height = 40)
```
...
<center><h3>Number of Diseased Coral Colonies (Raw Survey Data)</h3></center>
<br>
...
output$out_disease_count_plotly <- plotly::renderPlotly({
  ...
  shiny::validate(
    need(!is.null(rv_param_temp$disease_prev_subdat) & !is.null(rv_param_temp$disease_prev_subdat_transect), message = "From 'Data Filter Options' on the left, please select site(s), then press the green 'UPDATE DISEASE SUMMARIES' button"))
  
  shiny::req(!is.null(input$disease_free_ymax), !is.null(rv_param_temp$sel_disease_showRSS))
  
  rv_param_temp$disease_count_plot <- FuncPlotPointDisease(dat = switch((input$disease_by_transect == TRUE)+1, rv_param_temp$disease_prev_subdat, rv_param_temp$disease_prev_subdat_transect), metric = switch((input$disease_by_transect == TRUE)+1, "SiteDiseased", "TransectDiseased"), free_ymax = input$disease_free_ymax, y_text = "Number of Diseased Coral Colonies Detected", disease_by_transect = input$disease_by_transect)
...
<center><h3>Coral Disease Prevalence (Raw Survey Data)</h3></center>
<br>
```{r plot_disease_prev}
output$out_disease_prev_plotly <- plotly::renderPlotly({
  ...
  rv_param_temp$disease_prev_plot <- FuncPlotPointDisease(dat = switch((input$disease_by_transect == TRUE)+1, rv_param_temp$disease_prev_subdat, rv_param_temp$disease_prev_subdat_transect), metric = switch((input$disease_by_transect == TRUE)+1, "SitePrev", "TransectPrev"), free_ymax = input$disease_free_ymax, y_text = "% of Coral Colonies with Disease", disease_by_transect = input$disease_by_transect)
...
% Coral Disease Counts & Prevalence (stacked bar plots) {data-navmenu="Coral Disease per Site"}
======================================

Coral Disease Counts & Prevalence per Site
-------------------------------------
#### <font size="3">**These stacked bar plots show count of diseased colonies (left-side panel) and disease prevalence (right-side panel) for each selected site [PLOTS ARE SLOW TO DRAW - PLEASE BE PATIENT]**</font>

####
```{r}
renderPlot({ # separate validation statements so they are checked in sequence
  shiny::req(!is.null(rv$disease_df), !is.null(rv$colcounts_df), !is.null(rv_param_temp$disease_prev_subdat))

  disease_barplot_legend <-
    ggpubr::as_ggplot(cowplot::get_legend(
  ggplot(data.frame(x=1:3, y = 1:3, DiseaseStatus = c("Diseased", "NotDiseased", "Unknown")), aes(x = x, y = y, fill = DiseaseStatus)) +
    geom_col() + 
    scale_fill_manual(values = c("Diseased" = "red", "NotDiseased" = "black", "Unknown" = "yellow"), guide = guide_legend(override.aes = list(size=1.25))) + 
    theme_bw(base_size = 12) +
    theme(legend.position = "top",
          legend.text = element_text(size = 16),
          legend.title = element_text(size = 16))
  ))
  grid.arrange(disease_barplot_legend)
}, height = 40)
```
...
output$out_disease_count_stackplot <- renderPlot({
  ...
  disease_counts_dat <- rv_param_temp$disease_prev_subdat %>%
    ungroup() %>%
  ...
  FuncPlotDisease(dat = disease_counts_dat, y_col = "NumColonies", yaxis_title = "Number of Coral Colonies", title_text = "Count of diseased colonies per site, summed across transects")
},
  height = function(){shiny::validate(need(!is.null(rv$raw_dat), message = "Please import new data or load an existing summary RDS file"));shiny::validate(need(!is.null(rv_param_temp$disease_prev_subdat) & !is.null(rv_param_temp$disease_prev_subdat_transect), message = "From 'Data Filter Options' on the line graphs page, please select site(s), then press the green 'UPDATE DISEASE SUMMARIES' button"));shiny::req(!is.null(rv_param_temp$sel_disease_showRSS));length(rv_param_temp$sel_disease_showRSS)*350}
)
...
```{r barplot_disease_prev}
output$out_disease_prev_stackplot <- renderPlot({
  
  shiny::req(!is.null(rv$disease_df), !is.null(rv$colcounts_df), !is.null(rv_param_temp$disease_prev_subdat))
  
  disease_prev_dat <- rv_param_temp$disease_prev_subdat %>%
    ungroup() %>%
    dplyr::select(RSS, Year, SurvDate, SitePrev) %>%
    dplyr::mutate(
      Diseased = SitePrev,
      NotDiseased = 100 - SitePrev,
      Unknown = NA)
  
  disease_prev_dat$Unknown[is.na(disease_prev_dat$SitePrev)] <- 100
  disease_prev_dat %<>%
    dplyr::select(-SitePrev) %>%
    arrange(RSS, SurvDate) %>%
    dplyr::mutate(SurvDate = as.factor(SurvDate)) %>%
    tidyr::pivot_longer(cols = c("Diseased", "NotDiseased", "Unknown"), names_to = "DiseaseStatus", values_to = "PercentColonies")

  FuncPlotDisease(dat = disease_prev_dat, y_col = "PercentColonies", yaxis_title = "% of Coral Colonies with Disease", title_text = "Disease prevalence by site, summed across transects")
},
  height = function(){shiny::validate(need(!is.null(rv$raw_dat), message = "Please import new data or load an existing summary RDS file"));shiny::validate(need(!is.null(rv_param_temp$disease_prev_subdat) & !is.null(rv_param_temp$disease_prev_subdat_transect), message = "From 'Data Filter Options' on the line graphs page, please select site(s), then press the green 'UPDATE DISEASE SUMMARIES' button"));shiny::req(!is.null(rv_param_temp$sel_disease_showRSS));length(rv_param_temp$sel_disease_showRSS)*350}
)

plotOutput("out_disease_prev_stackplot")
```
  
</div>
</div>

Disease Counts & Prevalence (summed to SITE) {data-navmenu="Coral Disease per Site"}
======================================
#### <font size="3">**This table shows site-level disease count and prevalence.**</font>
...
output$out_disease_prev_tab <- DT::renderDT({
  ...
  shiny::req(!is.null(rv_param_temp$disease_prev_subdat))
  
  disease_prev_tab <- rv_param_temp$disease_prev_subdat %>%
    dplyr::select(-RSS) %>%
    dplyr::select(ParkCode, ReportingSite, ReportingSiteName, Site, IsActive, Year, SurvDate, Purpose, ProtocolVersion, ColCountsCertificationLevel, SiteDiseased, SiteNotDiseased, SitePrev, SiteStonyCoralCount1, SiteStonyCoralCount2, SiteStonyCoralCountDIFF, SiteFireCoralCount1, SiteFireCoralCount2, SiteFireCoralCountDIFF) %>%
    dplyr::mutate_at(c("ParkCode", "ReportingSite", "ReportingSiteName", "Site", "IsActive", "Purpose", "ProtocolVersion", "ColCountsCertificationLevel"), as.factor) %>%
    dplyr::mutate_at(c("Year", "SiteDiseased", "SiteNotDiseased", "SiteStonyCoralCount1", "SiteStonyCoralCount2", "SiteStonyCoralCountDIFF", "SiteFireCoralCount1", "SiteFireCoralCount2", "SiteFireCoralCountDIFF"), as.integer) %>%
    dplyr::rename(
...
Disease Counts & Prevalence (summed to TRANSECT) {data-navmenu="Coral Disease per Site"}
======================================
#### <font size="3">**This table shows transect-level disease count and prevalence.**</font>

```{r}
output$out_disease_prev_transect_tab <- DT::renderDT({
  Sys.sleep(0.6)
  
  shiny::validate(
    need(!is.null(rv$raw_dat), message = "Please import new data or load an existing summary RDS file"))
  
  shiny::req(!is.null(rv_param_temp$disease_prev_subdat_transect))
  
  disease_prev_transect_tab <- rv_param_temp$disease_prev_subdat_transect %>%
    dplyr::select(-RSS, -Site_Transect, -TransectSurveyID) %>%
    dplyr::select(ParkCode, ReportingSite, ReportingSiteName, Site, IsActive, Year, SurvDate, Purpose, ProtocolVersion, ColCountsCertificationLevel, Transect, TransectDiseased, TransectNotDiseased, TransectPrev, StonyCoralCount1, StonyCoralCount2, StonyCoralCountDIFF, FireCoralCount1, FireCoralCount2, FireCoralCountDIFF) %>%
    ...
    dplyr::mutate_at(c("Year", "TransectDiseased", "TransectNotDiseased", "StonyCoralCount1", "StonyCoralCount2", "StonyCoralCountDIFF", "FireCoralCount1", "FireCoralCount2", "FireCoralCountDIFF"), as.integer) %>%
    dplyr::rename(
...
dataTableOutput("out_disease_prev_transect_tab")
tags$style("#out_disease_prev_transect_tab{height:100vh;overflow-x:scroll;overflow-y:scroll}")
``` 
...
output$out_colcounts_tab <- DT::renderDT({
  ...
  colcounts_tab <- rv$colcounts_df %>% # for easier filtering
    ...
    dplyr::mutate_at(c("Transect", "StonyCoralCount1", "StonyCoralCount2", "FireCoralCount1", "FireCoralCount2"), as.integer)
