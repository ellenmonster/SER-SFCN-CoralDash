...
rv_param_temp <- reactiveValues(cover_subdat = NULL, avail_sublevs = NULL, sel_showRSS = NULL, sel_lev = NULL, sel_sublev = NULL, avail_diseases = NULL, transects_subdat = NULL, bleach_subdat = NULL, disease_prev_transect = NULL, disease_prev_site = NULL, disease_subdat = NULL)

rv <- readRDS("~/NPS PROJECTS_CURRENT/SFCN_CoralDash/TEST_partial_coralsummary.RDS") # <<<<<<<<<<<<<<<<<<<< FOR TESTING ONLY
...
FuncPlotPointCI <- function(dat, denom, sel_error, sel_splitplot, free_ymax, y_text, PC_blackwhite) {
  ...
  facet_ht <- 255 # 215
  add_on_ht <- 80 # 150, this is for legend
  ...
  gp <- gp %>% layout(
    legend = list(orientation = "h", xanchor = "center", yanchor = "bottom", y = 1.03, x = 0.5, yref = "plot"),
    margin = list(
      t=80,
      b=70,
      l= 100,
...
FuncPlotDiseaseTransect <- function(dat, y_col, yaxis_title, subtitle_text) {
...
observeEvent(eventExpr = input$button_update_disease, {
  ...
  # Filter colony counts data for the user-specified site
   ...
   disease_prev_site <- disease_prev_transect %>%
     ...
     dplyr::group_by(across(c(-Site_Transect, -TransectDiseased, -TransectNotDiseased, -StonyCoralCount1, -StonyCoralCount2, -FireCoralCount1, -FireCoralCount2, -TransectPrev))) %>%
     dplyr::summarize(
...
output$out_transects_plot <- plotly::renderPlotly({
  ...
  p <- ggplot(rv_param_temp$transects_subdat, aes(x = SurvDate, y = PercCov)) +
    labs(title = paste0("% Cover per Transect for ", toupper(input$sel_raw_categ), " at ", input$sel_raw_showRSS)) +
    geom_point() +
    geom_line() +
...
% Cover and Relative Cover (figs) {data-navmenu="Group Cover per Site"}
======================================  
Inputs {.sidebar data-width=300}
-------------------------------------
...
### >>>>>>>>>>>> PICK UP FROM HERE--1. create shared legend and remove legend from plots--also switch to subplots; convert all data tables to nested reactables and fix col widths--OR check data tbl widths with navbars; fix bleach plots to match cover plots as well; change transect plot name; add disease data to maps; fix explanatory text for all reactables; add informational text to sidebars; make better disease transect-level plots
...
#### <font size="3">**For each BLAH, this table shows BLAH BLAH **</font>

* Hover over a column name to see column definitions

* EXPLANATION OF CALCULATIONS

* Remember that a 95% confidence interval gives us an idea of confidence in the mean estimate -- this is not the same as the standard deviation of the samples. 

```{r}
output$tab_cover <- renderReactable({
  shiny::validate(
    need(!is.null(rv$raw_dat), message = "Please import new data or load an existing summary RDS file"))
  shiny::req(!is.null(rv_param_temp$cover_subdat))
  
  reactable(
    subset(rv_param_temp$cover_subdat, DenomGroup == "TransectCount"),
    resizable = TRUE, 
    searchable = TRUE,
    filterable = TRUE,
    striped = TRUE,
    ...
    pageSizeOptions = c(20, 30, 40, 50), 
    defaultPageSize = 20)
})

reactableOutput("tab_cover")
tags$style("#tab_cover{height:100vh;overflow-x:scroll;overflow-y:scroll}")
``` 
...
```{r}
output$tab_rel_cover <- renderReactable({
  shiny::validate(
    need(!is.null(rv$raw_dat), message = "Please import new data or load an existing summary RDS file"))
  shiny::req(!is.null(rv_param_temp$cover_subdat))
  
  reactable(
    subset(rv_param_temp$cover_subdat, DenomGroup == "CategoryCount"),
    resizable = TRUE, 
    searchable = TRUE,
    filterable = TRUE,
    striped = TRUE,
...
reactableOutput("tab_rel_cover")
tags$style("#tab_rel_cover{height:100vh;overflow-x:scroll;overflow-y:scroll}")
``` 

% Bleaching and Relative Bleaching (figs) {data-navmenu="Coral Bleaching per Site"}
======================================
Inputs {.sidebar data-width=300}
-------------------------------------
...
```{r}
output$tab_bleach <- renderReactable({
  shiny::validate(
    need(!is.null(rv$raw_dat), message = "Please import new data or load an existing summary RDS file"))
  shiny::req(!is.null(rv$bleach_CIs_df))
  
  reactable(
    subset(rv$bleach_CIs_df, DenomGroup == "TransectCount"),
    resizable = TRUE, 
    searchable = TRUE,
    filterable = TRUE,
    striped = TRUE,
...
reactableOutput("tab_bleach")
tags$style("#tab_bleach{height:100vh;overflow-x:scroll;overflow-y:scroll}")
``` 
...
```{r}
output$tab_rel_bleach <- renderReactable({
  shiny::validate(
    need(!is.null(rv$raw_dat), message = "Please import new data or load an existing summary RDS file"))
  shiny::req(!is.null(rv$bleach_CIs_df))
  
  reactable(
    subset(rv$bleach_CIs_df, DenomGroup == "CategoryCount"),
    resizable = TRUE, 
    searchable = TRUE,
    filterable = TRUE,
    striped = TRUE,
...
reactableOutput("tab_rel_bleach")
tags$style("#tab_rel_bleach{height:100vh;overflow-x:scroll;overflow-y:scroll}")
``` 

Coral Disease Summaries per Site (Plots) {data-navmenu="Disease Summaries"}
======================================
Inputs {.sidebar data-width=300}
-------------------------------------
...
wellPanel(
  ...
  renderUI({
    ...
    disease_RSS_choices <- rv$disease_df %>%
      ...
      {if(input$sel_disease_sitescale == "Site") dplyr::pull(., Site) else dplyr::pull(., ReportingSite)} %>% 
      unique(.)
    cat("line1149")
...
checkboxInput("sel_ShowDiseaseTransects",
                      label = "Show results per transect",
                      value = FALSE)
```
...
output$out_disease_plot <- plotly::renderPlotly({
  ...
  shiny::req(!is.null(rv$disease_df), !is.null(rv$colcounts_df), !is.null(rv_param_temp$disease_prev_transect), !is.null(rv_param_temp$disease_prev_site), !is.null(input$sel_ShowDiseaseTransects))
  
    # Data to display in plots and table
  if(input$sel_ShowDiseaseTransects == TRUE) {
    rv_param_temp$disease_subdat <- rv_param_temp$disease_prev_transect %>%
      ungroup() %>%
      dplyr::select(SurvDate, Site_Transect, Site, Transect, Diseased = TransectDiseased, NotDiseased = TransectNotDiseased) %>%
    arrange(Site_Transect, SurvDate) %>%
    dplyr::mutate(SurvDate = as.factor(SurvDate)) %>%
    tidyr::pivot_longer(cols = c("Diseased", "NotDiseased"), names_to = "DiseaseStatus", values_to = "NumColonies")
    
    plot_disease <- FuncPlotDiseaseTransect(dat = rv_param_temp$disease_subdat, y_col = "NumColonies", yaxis_title = "Number of coral colonies", subtitle_text = paste0("Count of diseased colonies at ", unique(rv_param_temp$disease_prev_transect$RSS), ", shown by transect - FOR SELECTED DISEASES ONLY"))
    ...
    plot_disease <- FuncPlotDiseaseSite(dat = disease_dat, y_col = "NumColonies", yaxis_title = "Number of coral colonies", subtitle_text = paste0("Count of diseased colonies at ", unique(rv_param_temp$disease_prev_site$RSS), ", summed across transects - FOR SELECTED DISEASES ONLY"))
  }
...
output$out_disease_prev_plot <- plotly::renderPlotly({
  ...
  shiny::req(!is.null(rv$disease_df), !is.null(rv$colcounts_df), !is.null(rv_param_temp$disease_prev_site), !is.null(rv_param_temp$disease_prev_transect), !is.null(input$sel_ShowDiseaseTransects))

    # Data to display in plots and table
  if(input$sel_ShowDiseaseTransects == TRUE) {
    disease_trans_prev_dat <- rv_param_temp$disease_prev_transect %>%
    ungroup() %>%
    dplyr::select(SurvDate, Site_Transect, Site, Transect, TransectPrev) %>%
    dplyr::mutate(
      Diseased = round(100 * TransectPrev, 1),
      NotDiseased = round(100*(1 - TransectPrev), 1),
      Unknown = NA)
  
  disease_trans_prev_dat$Unknown[is.na(disease_trans_prev_dat$TransectPrev)] <- 100
  disease_trans_prev_dat %<>%
    arrange(Site_Transect, SurvDate) %>%
    dplyr::mutate(SurvDate = as.factor(SurvDate)) %>%
    tidyr::pivot_longer(cols = c("Diseased", "NotDiseased", "Unknown"), names_to = "DiseaseStatus", values_to = "PercentColonies")

  plot_disease_prev <- FuncPlotDiseaseTransect(dat = disease_trans_prev_dat, y_col = "PercentColonies", yaxis_title = "% of colonies with disease", subtitle_text = paste0("Disease prevalence at ", unique(rv_param_temp$disease_prev_transect$RSS), ", shown by transect - FOR SELECTED DISEASES ONLY"))
  ...
  plot_disease_prev <- FuncPlotDiseaseSite(dat = disease_prev_dat, y_col = "PercentColonies", yaxis_title = "% of colonies with disease", subtitle_text = paste0("Disease prevalence at ", unique(rv_param_temp$disease_prev_transect$RSS), ", summed across transects - FOR SELECTED DISEASES ONLY"))
  }

  ggplotly(plot_disease_prev)
 
})
plotlyOutput("out_disease_prev_plot")
```
...
Reactables {.tabset .tabset-fade}
-------------------------------------

### Cover Data

```{r}
output$tab_react_raw <- renderReactable({
  shiny::validate(
    need(!is.null(rv$raw_dat), message = "Please import new data or load an existing summary RDS file"))
...
reactableOutput("tab_react_raw")
tags$style("#tab_react_raw{height:100vh;overflow-x:scroll;overflow-y:scroll}")
```  

### Disease Data

```{r}
output$tab_react_disease <- renderReactable({
  shiny::validate(
    need(!is.null(rv$disease_df), message = "Please import new disease data or load an existing summary RDS file"))
...
reactableOutput("tab_react_disease")
tags$style("#tab_react_disease{height:100vh;overflow-x:scroll;overflow-y:scroll}")
```

Data Tables
====================================

Data Tables {.tabset .tabset-fade}
-------------------------------------
...
output$out_colcounts_tab <- DT::renderDT({
  shiny::validate(
    need(!is.null(rv$colcounts_df), message = "Please import new colony counts data or load an existing summary RDS file"))
