...
rv_param_temp <- reactiveValues(cover_subdat = NULL, avail_sublevs = NULL, sel_showRSS = NULL, sel_bleach_showRSS = NULL, sel_lev = NULL, sel_sublev = NULL, avail_diseases = NULL, transects_subdat = NULL, sel_raw_dates = NULL, bleach_subdat = NULL, disease_subdat = NULL, disease_subdat_transect = NULL, sel_disease_showRSS = NULL, cover_plot = NULL, bleach_plot = NULL, lesion_site = NULL, lesion_transect = NULL)
...
FuncPlotPointDisease <- function(dat, metric, free_ymax, set_ymax, y_text, disease_by_transect) {
  ...
  if(disease_by_transect == TRUE){ # if separate line for each transect...
    p <- 
      ...
      geom_point(aes(text = paste0("Reporting Site: ", ReportingSite, "<br>Site: ", Site, "<br>Survey Date: ", SurvDate, "<br>Transect: ", Transect, "<br>Y-VALUE: ", PlotMetric, "<br><br>Number of Diseased Colonies: ", TransectDiseased, "<br>Avg Number of Colonies: ", StonyCoralCountAVG, "<br>Disease Prevalence: ", TransectPrev))) + # aesthetic text is for ggplotly tooltip. Gives warning that it's not recognized but it still works
    geom_line(aes(text = paste0("Reporting Site: ", ReportingSite, "<br>Site: ", Site, "<br>Survey Date: ", SurvDate, "<br>Transect: ", Transect, "<br>Y-VALUE: ", PlotMetric, "<br><br>Number of Diseased Colonies: ", TransectDiseased, "<br>Avg Number of Colonies: ", StonyCoralCountAVG, "<br>Disease Prevalence: ", TransectPrev)), group = "Transect") # need to add this grouping to geom_line, otherwise the text will cause it to group incorrectly. Need to add text b/c otherwise the line will not show when plotly tooltip is set to "text"
  } else {
    p <- 
      ...
      geom_line(aes(text = paste0("Reporting Site: ", ReportingSite, "<br>Site: ", Site, "<br>Survey Date: ", SurvDate, "<br>Y-VALUE: ", PlotMetric, "<br><br>Number of Diseased Colonies: ", SiteDiseased, "<br>Avg Number of Colonies: ", SiteStonyCoralCountAVG, "<br>Disease Prevalence: ", SitePrev)), group = "RSS")
...
FuncPlotPointLesion <- function(dat, metric, free_ymax, set_ymax, y_text, disease_by_transect) {
  ...
  if(disease_by_transect == TRUE){ # if separate line for each transect...
    p <- 
      ggplot(data = plot_dat, aes(x = SurvDate, y = PlotMetric, colour = Transect)) +
      scale_color_viridis_d(name = metric, direction=-1, guide = guide_legend(override.aes = list(size=1.25))) +
      geom_point(aes(text = paste0("Reporting Site: ", ReportingSite, "<br>Site: ", Site, "<br>Survey Date: ", SurvDate, "<br>Transect: ", Transect, "<br>Y-VALUE: ", PlotMetric, "<br><br>Number of Disease Lesions: ", TransectLesionCounts, "<br>Summed Lesion Area: ", TransectLesionArea))) + # aesthetic text is for ggplotly tooltip. Gives warning that it's not recognized but it still works
    geom_line(aes(text = paste0("Reporting Site: ", ReportingSite, "<br>Site: ", Site, "<br>Survey Date: ", SurvDate, "<br>Transect: ", Transect, "<br>Y-VALUE: ", PlotMetric, "<br><br>Number of Disease Lesions: ", TransectLesionCounts, "<br>Summed Lesion Area: ", TransectLesionArea)), group = "Transect") # need to add this grouping to geom_line, otherwise the text will cause it to group incorrectly. Need to add text b/c otherwise the line will not show when plotly tooltip is set to "text"
  } else {
    p <- 
      ggplot(data = plot_dat, aes(x = SurvDate, y = PlotMetric)) +
      geom_point(aes(text = paste0("Reporting Site: ", ReportingSite, "<br>Site: ", Site, "<br>Survey Date: ", SurvDate, "<br>Y-VALUE: ", PlotMetric, "<br><br>Number of Disease Lesions: ", SiteLesionCounts, "<br>Summed Lesion Area: ", SiteLesionArea))) + # aesthetic text is for ggplotly tooltip. Gives warning that it's not recognized but it still works
      geom_line(aes(text = paste0("Reporting Site: ", ReportingSite, "<br>Site: ", Site, "<br>Survey Date: ", SurvDate, "<br>Y-VALUE: ", PlotMetric, "<br><br>Number of Disease Lesions: ", SiteLesionCounts, "<br>Summed Lesion Area: ", SiteLesionArea)), group = "RSS")
...
FuncPlotBleach <- function(dat, yaxis_title, free_ymax) {
  ...
  plots_list <- lapply(sort(unique(dat$RSS)), function(i) {
    ...
    p <- ggplot(subset(plot_dat, RSS == i), aes(x = SurvDate, y = EstimCov, fill = NumerLevel)) +
...
# saveRDS(isolate(reactiveValuesToList(rv_param_temp)), "temp_rv_param_temp.RDS")
  ...
  # Filter colony counts data for the user-specified RSS and calculate avg count
   ...
   disease_subdat <- disease_transect %>%
     dplyr::select(ParkCode, RSS, ReportingSite, ReportingSiteName, Site, IsActive, Year, SurvDate, Purpose, ProtocolVersion, ColCountsCertificationLevel, TransectDiseased, TransectNotDiseased, StonyCoralCountAVG) %>%
     dplyr::group_by(ParkCode, RSS, ReportingSite, ReportingSiteName, Site, IsActive, Year, SurvDate, Purpose, ProtocolVersion, ColCountsCertificationLevel) %>%
     dplyr::summarize(
   ...
   lesion_transect <- diseases %>%
     ...
     dplyr::select(ParkCode, ReportingSite, Site, IsActive, Transect, SurvDate, TransectSurveyID, Purpose, ProtocolVersion, DiseaseCertificationLevel, RSS, TransectLesionCounts, TransectLesionArea) %>%
     dplyr::distinct()
   
   lesion_site <- lesion_transect %>%
     dplyr::select(ParkCode, ReportingSite, Site, IsActive, SurvDate, Purpose, ProtocolVersion, DiseaseCertificationLevel, RSS, TransectLesionCounts, TransectLesionArea) %>%
     dplyr::group_by(ParkCode, ReportingSite, Site, IsActive, SurvDate, Purpose, ProtocolVersion, DiseaseCertificationLevel, RSS) %>%
     dplyr::summarize(
   ...
   rv_param_temp$disease_subdat <- disease_subdat
   rv_param_temp$disease_subdat_transect <- disease_transect
   rv_param_temp$lesion_site <- lesion_site
...
<span style="font-size:18px; color:red;">
    PLOTS ARE SLOW TO DRAW - PLEASE BE PATIENT 
</span>
...
output$out_bleach_stackplot <- renderPlot({
  ...
  shiny::req(!is.null(rv$bleach_CIs_df), !is.null(input$bleach_RSS_active), !is.null(input$sel_bleach_episodic), !is.null(input$sel_bleach_sitescale), !is.null(input$bleach_free_ymax))

  FuncPlotBleach(dat = subset(rv_param_temp$bleach_subdat, DenomGroup == "TransectCount"), yaxis_title = "% Cover of Site", free_ymax = input$bleach_free_ymax)
},
...
output$out_bleach_stackplot2 <- renderPlot({
  ...
  FuncPlotBleach(dat = subset(rv_param_temp$bleach_subdat, DenomGroup == "CategoryCount"), yaxis_title = "% Cover of Corals", free_ymax = TRUE)
},
      height = function(){shiny::validate(need(!is.null(rv$raw_dat), message = "Please import new data or load an existing summary RDS file"));shiny::validate(need(!is.null(rv_param_temp$bleach_subdat), message = "From 'Data Filter Options' on the line graphs page, please select site(s), then press the green 'UPDATE BLEACH SUMMARIES' button"));shiny::req(!is.null(rv_param_temp$sel_bleach_showRSS));length(rv_param_temp$sel_bleach_showRSS)*350}
  )
...
renderUI({
  shiny::req(!is.null(rv_param_temp$disease_subdat), input$disease_free_ymax == FALSE) # this option is only shown when y-axes scales are not allowed to vary among plots
  top_count <-max(rv_param_temp$disease_subdat$SiteDiseased, na.rm = TRUE) %>% plyr::round_any(., 5, f = ceiling)
...
renderUI({
  shiny::req(!is.null(rv_param_temp$disease_subdat_transect), input$disease_free_ymax == FALSE) # this option is only shown when y-axes scales are not allowed to vary among plots
  top_prev <-max(rv_param_temp$disease_subdat_transect$TransectPrev, na.rm = TRUE) %>% plyr::round_any(., 0.2, f = ceiling)
...
Coral Disease
-------------------------------------
#### <font size="3">**These line graphs show count of diseased colonies (left-side panel) and disease prevalence (right-side panel) by transect or site**</font>
...
<span style="font-size:14px; color:red;">
    NOTE: Disease prevalence is calculated as NUMBER OF CORAL COLONIES WITH DISEASE (numerator) divided by the COUNT OF STONY CORAL COLONIES (denominator--typically, average of two independent counts), expressed as a percentage. Numerator and denominator counts may be conducted by different persons. 
</span>
...
output$out_disease_count_plotly <- plotly::renderPlotly({
  ...
  shiny::validate(
    need(!is.null(rv_param_temp$disease_subdat) & !is.null(rv_param_temp$disease_subdat_transect), message = "From 'Data Filter Options' on the left, please select site(s) and disease(s), then press the green 'UPDATE DISEASE SUMMARIES' button"))
  
  shiny::req(!is.null(input$disease_free_ymax), !is.null(rv_param_temp$sel_disease_showRSS))
  
  rv_param_temp$disease_count_plot <- FuncPlotPointDisease(dat = switch((input$disease_by_transect == TRUE)+1, rv_param_temp$disease_subdat, rv_param_temp$disease_subdat_transect), metric = switch((input$disease_by_transect == TRUE)+1, "SiteDiseased", "TransectDiseased"), free_ymax = input$disease_free_ymax, set_ymax = input$set_disease_count_ymax, y_text = "Number of Diseased Coral Colonies Detected", disease_by_transect = input$disease_by_transect)
...
output$out_disease_prev_plotly <- plotly::renderPlotly({
  ...
  rv_param_temp$disease_prev_plot <- FuncPlotPointDisease(dat = switch((input$disease_by_transect == TRUE)+1, rv_param_temp$disease_subdat, rv_param_temp$disease_subdat_transect), metric = switch((input$disease_by_transect == TRUE)+1, "SitePrev", "TransectPrev"), free_ymax = input$disease_free_ymax, set_ymax = input$set_disease_prev_ymax, y_text = "% of Coral Colonies with Disease", disease_by_transect = input$disease_by_transect)
...
Lesion Counts & Area (line graphs) {data-navmenu="Coral Disease per Site"}
======================================
Lesions
-------------------------------------
#### <font size="3">**These line graphs show count of disease lesions (left-side panel) and area of disease lesions (right-side panel) by transect or site**</font>

* Counts and area are summaries of the survey data collected from sample transects, without estimates of statistical error.

* All coral colonies that overlap survey transects are included. See protocol for how "coral colony" is defined.

<span style="font-size:14px; color:red;">
    NOTE: Lesion area is not standardized by total coral area. 
</span>
...
<center><h3>Number of Disease Lesions (Raw Survey Data)</h3></center>
<br>
```{r plot_lesion_count}
output$out_lesion_count_plotly <- plotly::renderPlotly({
  ...
  shiny::validate(
    need(!is.null(rv_param_temp$lesion_site) & !is.null(rv_param_temp$lesion_transect), message = "From 'Data Filter Options' on the left, please select site(s) and disease(s), then press the green 'UPDATE DISEASE SUMMARIES' button"))
  
  shiny::req(!is.null(rv_param_temp$sel_disease_showRSS))
  
  rv_param_temp$lesion_count_plot <- FuncPlotPointLesion(dat = switch((input$disease_by_transect == TRUE)+1, rv_param_temp$lesion_site, rv_param_temp$lesion_transect), metric = switch((input$disease_by_transect == TRUE)+1, "SiteLesionCounts", "TransectLesionCounts"), free_ymax = TRUE, set_ymax = NULL, y_text = "Number of Disease Lesions Detected", disease_by_transect = input$disease_by_transect)
...
<center><h3>Lesion Area (Raw Survey Data)</h3></center>
<br>
```{r plot_lesion_area}
output$out_lesion_area_plotly <- plotly::renderPlotly({
  ...
  rv_param_temp$lesion_area_plot <- FuncPlotPointLesion(dat = switch((input$disease_by_transect == TRUE)+1, rv_param_temp$lesion_site, rv_param_temp$lesion_transect), metric = switch((input$disease_by_transect == TRUE)+1, "SiteLesionArea", "TransectLesionArea"), free_ymax = TRUE, set_ymax = NULL, y_text = "Summed Lesion Area", disease_by_transect = input$disease_by_transect)
...
```{r}
renderPlot({ # separate validation statements so they are checked in sequence
  shiny::req(!is.null(rv$disease_df), !is.null(rv$colcounts_df), !is.null(rv_param_temp$disease_subdat))

  disease_barplot_legend <-
    ggpubr::as_ggplot(cowplot::get_legend(
  ggplot(data.frame(x=1:2, y = 1:2, DiseaseStatus = c("Diseased", "NotDiseased")), aes(x = x, y = y, fill = DiseaseStatus)) +
    geom_col() + 
    scale_fill_manual(values = c("Diseased" = "red", "NotDiseased" = "black"), guide = guide_legend(override.aes = list(size=1.25))) + 
    theme_bw(base_size = 12) +
    theme(legend.position = "top",
          legend.text = element_text(size = 16),
          legend.title = element_text(size = 16))
...
<center><h3>Number of Diseased Coral Colonies (in Survey Transects)</h3></center>
<br>
...
output$out_disease_count_stackplot <- renderPlot({
  ...
  disease_counts_dat <- rv_param_temp$disease_subdat %>%
    ungroup() %>%
  ...
  height = function(){shiny::validate(need(!is.null(rv$raw_dat), message = "Please import new data or load an existing summary RDS file"));shiny::validate(need(!is.null(rv_param_temp$disease_subdat) & !is.null(rv_param_temp$disease_subdat_transect), message = "From 'Data Filter Options' on the line graphs page, please select site(s) and disease(s), then press the green 'UPDATE DISEASE SUMMARIES' button"));shiny::req(!is.null(rv_param_temp$sel_disease_showRSS));length(rv_param_temp$sel_disease_showRSS)*350}
)
...
<center><h3>Coral Disease Prevalence (in Survey Transects)</h3></center>
<br>
...
output$out_disease_prev_stackplot <- renderPlot({
  ...
  disease_prev_dat <- rv_param_temp$disease_subdat %>%
    ungroup() %>%
  ...
  disease_prev_dat %<>%
    ...
    dplyr::mutate(SurvDate = as.factor(SurvDate)) %>%
    tidyr::pivot_longer(cols = c("Diseased", "NotDiseased"), names_to = "DiseaseStatus", values_to = "PercentColonies")
...
output$out_disease_prev_tab <- DT::renderDT({
  ...
  shiny::req(!is.null(rv_param_temp$disease_subdat))
  
  disease_prev_tab <- rv_param_temp$disease_subdat %>%
    dplyr::select(-RSS) %>%
    dplyr::select(ParkCode, ReportingSite, ReportingSiteName, Site, IsActive, Year, SurvDate, Purpose, ProtocolVersion, ColCountsCertificationLevel, SiteDiseased, SiteNotDiseased, SiteStonyCoralCountAVG, SitePrev) %>%
...
output$out_disease_transect_tab <- DT::renderDT({
  ...
  shiny::req(!is.null(rv_param_temp$disease_subdat_transect))
  
  disease_transect_tab <- rv_param_temp$disease_subdat_transect %>%
    ...
    dplyr::rename(
      ...
      AvgNumColongies = StonyCoralCountAVG,
