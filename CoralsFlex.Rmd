...
FuncPlotStackedBar <- function(dat, y_col, yaxis_title, free_ymax, color_vec, fill_col, metric_name) {
  # Function to stack bar plots
  #
  # Args:
  #   dat:  The (bleach or disease) data frame
  #
  # Returns:
  #   Stacked bar plots
  # 
  if(free_ymax == FALSE) { # If one y-max for all plots, calculate what that should be
    ymax <- dat %>% select(RSS, SurvDate, !!as.name(y_col)) %>% dplyr::group_by(RSS, SurvDate) %>% dplyr::mutate(TotY = sum(!!as.name(y_col))) %>% pull(TotY) %>% max(., na.rm = TRUE) %>% plyr::round_any(., 2.5, f = ceiling)
  } else {
    ymax <- NA
  }
  # 
  # template <- expand(dat, RSS, Year, NumerLevel)
  # plot_dat <- full_join(template, dat)
  # plot_dat$EstimCov[is.na(plot_dat$EstimCov)] <- 0

  plot_dat <- dat %>%
    dplyr::mutate(SurvDate = as.factor(SurvDate),
                  Year = as.factor(Year)) # this forces every plot to show every year
  ...
  for(i in sort(unique(plot_dat$RSS))) {
    ...
    yr_labels = paste0("  ", levels(sub_dat$Year), "  ") # add space around year labels
    names(yr_labels) <- levels(sub_dat$Year)
    
    subtitle_list[[plot_num]] = list(
      text = paste0("Coral ", metric_name, " at Site: ", i), 
      x = 0.5,
      xanchor = "middle",
      xref = "paper",
      y = 0,
    ...
    p <- ggplot(sub_dat, aes_string(x = "SurvDate", y = y_col, fill = fill_col)) +
      geom_col(colour="black", size = 0.1, position = position_stack(reverse = TRUE), na.rm = FALSE) +
  ...
  plotly_out <-
    plotly::subplot( # this automatically converts to plotly
      ...
      titleY = TRUE,
      # which_layout = 1,
      margin = c(0, 0, 0.18/length(plots_list), 0.18/length(plots_list))) %>%
    layout(
      margin = list( # this is the page margin
        t=20,
        b=30,
        l= 100,
        r=20),
      height = length(plots_list) * 280 + 64) %>% # height here is deprecated, but having trouble getting reactive value in plotlyOutput
...
saveRDS(isolate(reactiveValuesToList(input)), paste0("temp_input.RDS"))
    ...
    rv_param_temp$bleach_pc_plot <- FuncPlotStackedBar(dat = subset(rv_param_temp$bleach_subdat, DenomGroup == "TransectCount"), yaxis_title = "% Cover of Site", free_ymax = input$bleach_free_ymax, color_vec = c("NoData" = "orange", "UNBL" = "gray25", "BL1" = "gray45", "BL2" = "gray55", "BL3" = "gray75", "BL4" = "gray92"), y_col = "EstimCov", fill_col = "NumerLevel", metric_name = "Bleaching")
  
  rv_param_temp$bleach_rc_plot <- FuncPlotStackedBar(dat = subset(rv_param_temp$bleach_subdat, DenomGroup == "CategoryCount"), yaxis_title = "% Cover of Corals", free_ymax = TRUE, color_vec = c("NoData" = "orange", "UNBL" = "gray25", "BL1" = "gray45", "BL2" = "gray55", "BL3" = "gray75", "BL4" = "gray92"), y_col = "EstimCov", fill_col = "NumerLevel", metric_name = "Bleaching")
...
output$out_transects_tab <- DT::renderDT({
  ...
  num_transects <- nrow(compare_dat) # May include NA's
...
<span style="font-size:18px; color:red;">
    INTERACTIVE STACKED BAR PLOTS ARE PAINFULLY SLOW TO DRAW - PLEASE BE PATIENT 
</span>
...
<center>Survey years are shown across top of each subplot; site name below each subplot</center>
<br>
```{r barplot_bleachPC}
...
output$out_disease_count_plotly <- plotly::renderPlotly({
  ...
  rv_param_temp$disease_count_plot$plot
})
...
output$out_disease_count_stackplot <- renderPlotly({
  ...
  FuncPlotStackedBar(dat = disease_counts_dat, y_col = "NumColonies", yaxis_title = "Number of Coral Colonies", free_ymax = TRUE, color_vec = c("Diseased" = "red", "NotDiseased" = "black"), fill_col = "DiseaseStatus", metric_name = "Disease")
})

plotlyOutput("out_disease_count_stackplot")
```
