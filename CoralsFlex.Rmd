...
cat("line30")
# 
# rv <- readRDS("~/NPS PROJECTS_CURRENT/SFCN_CoralDash/TEST_coralsummary.RDS") # <<<<<<<<<<<<<<<<<<<< FOR TESTING ONLY
...
FuncPlotPointCI <- function(dat, denom, sel_error, sel_splitplot, free_ymax, y_text, which_showRSS, PC_blackwhite) {
  ...
  # assign colors
    ...
    if(PC_blackwhite) {
      ...
      if(which_showRSS == "bleach") {
        p <- p + 
          scale_color_manual(values = c("NoData" = "orange", "UNBL" = "gray25", "BL1" = "gray45", "BL2" = "gray55", "BL3" = "gray75", "BL4" = "gray92"), guide = guide_legend(override.aes = list(size=1.25)))
...
FuncPlotBleach <- function(dat, yaxis_title) {
  ...
  plots_list <- lapply(sort(unique(dat$RSS)), function(i) {
    ...
    scale_fill_manual(values = c("NoData" = "orange", "UNBL" = "gray25", "BL1" = "gray45", "BL2" = "gray55", "BL3" = "gray75", "BL4" = "gray92"), guide = "none") +
  ...
  plots_page <- patchwork::wrap_plots(plots_list, ncol = 1) +
    patchwork::plot_annotation(
      title = "For surveys from Oct. 2005 to present, coral data with NA entry for 'Bleaching Code' are considered 'Unbleached' (UNBL)",
      theme = theme(
...
cat("line795")
    ...
    # select_opts <- c(
    #               switch(all(!is.null(input$sel_raw_dates[[1]]),  input$sel_raw_dates[[1]]>=min(date_opts, na.rm = TRUE))+1, min(date_opts, na.rm = TRUE), input$sel_raw_dates[[1]]),
    #               switch(all(!is.null(input$sel_raw_dates[[2]]), input$sel_raw_dates[[2]]<=max(date_opts, na.rm = TRUE))+1, max(date_opts, na.rm = TRUE), input$sel_raw_dates[[2]])
                # )
...
rv_param_temp$cover_plot <- FuncPlotPointCI(dat = rv_param_temp$cover_subdat, denom = "TransectCount", sel_error = input$sel_error, sel_splitplot = input$sel_splitplot, free_ymax = input$free_ymax, y_text = paste0("Mean % Cover (", input$sel_error, ")"), which_showRSS = "cover", PC_blackwhite = input$PC_blackwhite)
...
output$out_RC_plotly <- plotly::renderPlotly({
  ...
  FuncPlotPointCI(dat = rv_param_temp$cover_subdat, denom = "CategoryCount", sel_error = input$sel_error, sel_splitplot = input$sel_splitplot, free_ymax = input$free_ymax, y_text = paste0("Mean % Relative Cover (", input$sel_error, ")"), which_showRSS = "cover", PC_blackwhite = input$PC_blackwhite)$plot %>% hide_legend(.)
...
% Cover (data & est'd CI) {data-navmenu="Group Cover per Site"}
======================================  
...
Relative % Cover (data & est'd CI) {data-navmenu="Group Cover per Site"} 
======================================  
...
output$out_bleachPC_plotly <- plotly::renderPlotly({
  ...
  rv_param_temp$bleach_plot <- FuncPlotPointCI(dat = rv_param_temp$bleach_subdat, denom = "TransectCount", sel_error = input$sel_bleach_error, sel_splitplot = "one_loc_many_sub", free_ymax = input$bleach_free_ymax, y_text = paste0("% Cover of Site (", input$sel_bleach_error, ")"), which_showRSS = "bleach", PC_blackwhite = FALSE)
...
output$out_bleachRC_plotly <- plotly::renderPlotly({
  ...
  FuncPlotPointCI(dat = rv_param_temp$bleach_subdat, denom = "CategoryCount", sel_error = input$sel_bleach_error, sel_splitplot = "one_loc_many_sub", free_ymax = input$bleach_free_ymax, y_text = paste0("% Cover of Corals (", input$sel_bleach_error, ")"), which_showRSS = "bleach", PC_blackwhite = FALSE)$plot %>% hide_legend(.)
...
# tags$style("#out_bleach_stackplot{height:100vh;overflow-y:scroll}")
...
% Bleaching (data & est'd CI) {data-navmenu="Coral Bleaching per Site"}
======================================
...
Relative % Bleaching (data & est'd CI) {data-navmenu="Coral Bleaching per Site"}
======================================
...
-------------------------------------
#### <font size="3">**These stacked bar plots show count of diseased colonies (left-side panel) and disease prevalence (right-side panel) for each selected site**</font>
