...
FuncBootSamples <- function(dat, start_col, n_bootsamples = 1000000) { 
...
FuncBootDraws <- function(boot_dat, actual_dat, start_col, groups_df, combos_df, n_bootreps = 10000) {
  ...
  samps <- plyr::rbind.fill(samps, actual_dat %>% dplyr::mutate(BootRep = 0)) %>%
   ...
   #>>>>>>>>> PICK UP FROM HERE FOR BLEACHING
  # This is the master data frame from which to calculate % cover at different grouping levels.
  samps_master <- samps %>%
    tidyr::gather(key = "Taxon", value = "Count", -Site, -SurvDate, -AdjTot, -BootRep, -RowID, -TransectSurveyID) %>% # convert to long format
    dplyr::left_join(groups_df, by = "Taxon")
  
  # samps_master <- samps %>%
  #   tidyr::gather(key = "BleachingCode", value = "Count", -Site, -SurvDate, -AdjTot, -BootRep, -RowID, -TransectSurveyID) %>% # convert to long format
  #   dplyr::left_join(groups_df, by = "BleachingCode")
...
site_CIs_list <- apply(unique(boot_site_dat[c("Site", "SurvDate")]), 1, FUN = function(x) { 
  ...
  boot_samples <- FuncBootSamples(dat = subdat, start_col = start_col, n_bootsamples = 10000 ) #<<<<<<<<<<< SMALL NUMBER JUST FOR TESTING
  ...
  boot_site_estimates <- FuncBootDraws(boot_dat = boot_samples, start_col = start_col, actual_dat = actual_dat, groups_df = grps_df, combos_df = combos_df, n_bootreps = 1000) #<<<<<<<<<<< SMALL NUMBER JUST FOR TESTING
